...
void aStar(puzzle *prob)
{
	...
	if (info[i]->board != NULL)
	{
		...
		// print goal
		if ((finishstate->board[9]) == 1)
			cout << "Move: Up";
		else if ((finishstate->board[9]) == 2)
			cout << "Move: Down";
		else if ((finishstate->board[9]) == 3)
			cout << "Move: Left";
		else if ((finishstate->board[9]) == 4)
			cout << "Move: Right";
...
{
	cout << endl
		 << "This puzzle now like this : " << endl;
	for (int i = 0; i < 9; i++)
	{
		if (a->board[i] != 0)
		{
